...
export class PhysicsWorld {
    ...
    public addMesh(mesh: Mesh): void {
        ...
        for (let i = 0; i < positions.length; i += 9) {
            console.log(i)
            const v1 = new this._ammo.btVector3(positions[i + 0], positions[i + 1], positions[i + 2])
            const v2 = new this._ammo.btVector3(positions[i + 3], positions[i + 4], positions[i + 5])
            const v3 = new this._ammo.btVector3(positions[i + 6], positions[i + 7], positions[i + 7])


            physicsMesh.addTriangle(v1, v2, v3, true)
        }

        const shape = new this._ammo.btBvhTriangleMeshShape(physicsMesh, true, true)
        ...
        const bci = new this._ammo.btRigidBodyConstructionInfo(groundMass, groundMotionState, shape)
        const body = new this._ammo.btRigidBody(bci)

        this._ammoPhysicsWorld.addRigidBody(body)
    }

    public rayCast(startPosition: Vector3, destinationPosition: Vector3): RayHitResult {
    ...
    private _initWorld(): void {
           const collisionConfiguration = new this._ammo.btDefaultCollisionConfiguration()
            ...
            this._ammoPhysicsWorld = new this._ammo.btDiscreteDynamicsWorld(dispatcher, broadphase, solver, collisionConfiguration)
